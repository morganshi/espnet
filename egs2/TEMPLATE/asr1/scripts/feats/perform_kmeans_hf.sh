    ...
    for dset in ${_dsets}; do
        ...
        ${_cmd} JOB=1:${_nj} ${_logdir}/dump_features.JOB.log \
            ${python} pyscripts/feats/dump_hf_feature.py \
                --feature_conf "{"type": "hf", "conf": {"download_dir": "/data/mohan/workdir/espnet/egs2/ogi/asr2/exp/wavlm-large-ogi-fullfinetune/", "multilayer_feature": false, "layer": ${layer}}}" \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    for dset in "${train_set}" "${dev_set}" ${other_sets}; do
        ...
        if ${storage_save_mode}; then
            ...
            _opts+="--feature_conf \"{"type": "hf", "conf": {"download_dir": "/data/mohan/workdir/espnet/egs2/ogi/asr2/exp/wavlm-large-ogi-fullfinetune", "multilayer_feature": false, "layer": ${layer}}}\" "
        ...
        ${_cmd} JOB=1:${_nj} "${_dump_dir}"/logdir/inference_pseudo_labels_km${nclusters}.JOB.log \
            ${python} pyscripts/feats/dump_km_label_hf.py \
                ${_opts} \
                --audio_sample_rate "${audio_sample_rate}" \
                --km_path "${km_dir}/km_${nclusters}.mdl" \
                --out_filetype "mat" \
