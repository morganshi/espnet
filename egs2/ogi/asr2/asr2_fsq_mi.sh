...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    if [ "${tokenization_choice}" == "ssl" ]; then
        ...
        mkdir -p ${data_extract}/FSQ/FSQ/${train_set}
        mkdir -p ${data_extract}/FSQ/FSQ/${_dev_set}
        mkdir -p ${data_extract}/FSQ/FSQ/${test_sets}
        ...
        if [ "${src_case}" = ts ]; then
            ...
            for dset in "${train_set}" ${train_sp_sets} "${_dev_set}" ${test_sets}; do
                awk '
                    ...
                    > "${data_extract}/FSQ/FSQ/${dset}"/text.${src_case}.${src_lang}
            done
        ...
        if ${eval_valid_set}; then
            utils/copy_data_dir.sh --validate_opts --non-print ${data_audio}/org/${valid_set} \
                ${data_extract}/FSQ/FSQ/${valid_set}
            cp ${data_extract}/FSQ/FSQ/org/${valid_set}/text.${src_case}.${src_lang} \
                ${data_extract}/FSQ/FSQ/${valid_set}
            cp ${data_extract}/FSQ/FSQ/org/${valid_set}/text.${tgt_case}.${tgt_lang} \
                ${data_extract}/FSQ/FSQ/${valid_set}

            utils/fix_data_dir.sh --utt_extra_files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
                "${data_extract}/FSQ/FSQ/${dset}"
        fi

        if [ -n "${speed_perturb_factors}" ]; then
            _dirs="${data_extract}/FSQ/FSQ/${train_set} "
            for factor in ${speed_perturb_factors}; do
                if python3 -c "assert ${factor} != 1.0" 2>/dev/null; then
                    _dirs+="${data_extract}/FSQ/FSQ/${train_set}_sp${factor} "
                fi
            done
            utils/combine_data.sh \
                --extra_files "feats.scp utt2num_frames text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
                "${data_extract}/FSQ/FSQ/${train_set}_sp" ${_dirs}
        fi
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            for extra_file in ${utt_extra_files}; do
                ...
                for single_file in "${data_extract}/FSQ/FSQ/${dset}"/*; do
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    if [ "${num_splits_asr}" -gt 1 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text.${src_case}.${src_lang},src_text1,${input_src_type} "
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text.ts.wavlm_large_finetune_24_km2000,src_text2,${input_src_type} "
        ...
        _opts+="--train_shape_file /data/mohan/workdir/espnet/egs2/ogi/asr2/exp/asr_stats_raw_ts_wavlm_large_finetune_24_km2000_char_bpe200/train/src_text_shape.char "
    ...
    ${python} -m espnet2.bin.launch \
        ...
        --multiprocessing_distributed true -- \
        ${python} -m espnet2.bin.mt_mi_train \
            --use_preprocessor true \
            --bpemodel "${tgt_bpemodel}" \
            --token_type "${tgt_token_type}" \
            --token_list "${tgt_token_list}" \
            --src_bpemodel1 "${src_bpemodel}" \
            --src_token_type1 "${src_token_type}" \
            --src_token_list1 "${src_token_list}" \
            --src_bpemodel2 none \
            --src_token_type2 "char" \
            --src_token_list2 "/data/mohan/workdir/espnet/egs2/ogi/asr2/data/token_list/char_wavlm_large_finetune_24_km2000/src_tokens.txt" \
            --non_linguistic_symbols "${nlsyms_txt}" \
            --cleaner "${cleaner}" \
            --g2p "${g2p}" \
            --valid_data_path_and_name_and_type "${_asr_valid_dir}/text.${tgt_case}.${tgt_lang},text,text" \
            --valid_data_path_and_name_and_type "${_asr_valid_dir}/text.${src_case}.${src_lang},src_text1,${input_src_type}" \
            --valid_data_path_and_name_and_type "${_asr_valid_dir}/text.ts.wavlm_large_finetune_24_km2000,src_text2,${input_src_type}" \
            ...
            --valid_shape_file "/data/mohan/workdir/espnet/egs2/ogi/asr2/exp/asr_stats_raw_ts_wavlm_large_finetune_24_km2000_char_bpe200/valid/src_text_shape.char" \
            --resume true \
