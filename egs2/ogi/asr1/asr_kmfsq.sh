...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if [ "${num_splits_asr}" -gt 1 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${_scp},speech,${_type} "
        _opts+="--train_data_path_and_name_and_type /data/mohan/workdir/espnet/egs2/ogi/asr2/dump/extracted/wavlm_large_finetune/layer24/train/pseudo_labels_km2000_rm.txt,km_id,text_int "
        _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
        _opts+="--train_shape_file /data/mohan/workdir/espnet/egs2/ogi/asr2/exp/asr_stats_raw_ts_wavlm_large_finetune_24_km2000_char_bpe200/train/src_text_shape "
    ...
    ${python} -m espnet2.bin.launch \
        ...
        --multiprocessing_distributed true -- \
        ${python} -m espnet2.bin.asr_kmfsq_train \
            --use_preprocessor true \
            --bpemodel "${bpemodel}" \
            --token_type "${token_type}" \
            --token_list "${token_list}" \
            ...
            --valid_data_path_and_name_and_type "/data/mohan/workdir/espnet/egs2/ogi/asr2/dump/extracted/wavlm_large_finetune/layer24/dev/pseudo_labels_km2000_rm.txt,km_id,text_int" \
            --valid_shape_file "${asr_stats_dir}/valid/speech_shape" \
            --valid_shape_file "/data/mohan/workdir/espnet/egs2/ogi/asr2/exp/asr_stats_raw_ts_wavlm_large_finetune_24_km2000_char_bpe200/valid/src_text_shape" \
            --resume true \
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            ${python} -m espnet2.bin.asr_kmfsq_inference \
                --batch_size ${batch_size} \
                --ngpu "${_ngpu}" \
                --data_path_and_name_and_type "${_data}/${_scp},speech,${_type}" \
                --data_path_and_name_and_type "${_data}/pseudo_labels_km2000_rm.txt,km_id,text_int" \
...
if [ ${stage} -le 14 ] && [ ${stop_stage} -ge 14 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    log "Stage 12: Decoding: training_dir=${asr_exp}"
    ...
    inference_tag='fsq_'$inference_tag
    ...
    for dset in ${_dsets}; do
        ...
        rm -f "${_logdir}/*.log"
        # shellcheck disable=SC2046,SC2086
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/fsq_inference.JOB.log \
            ${python} -m espnet2.bin.fsq_inference_token \
                --batch_size ${batch_size} \
                --ngpu "${_ngpu}" \
                --data_path_and_name_and_type "${_data}/${_scp},speech,${_type}" \
                --key_file "${_logdir}"/keys.JOB.scp \
...
if [ ${stage} -le 15 ] && [ ${stop_stage} -ge 15 ] && ! [[ " ${skip_stages} " =~ [[:space:]]14[[:space:]] ]]; then
...
if [ ${stage} -le 16 ] && [ ${stop_stage} -ge 16 ] && ! [[ " ${skip_stages} " =~ [[:space:]]15[[:space:]] ]]; then
    [ -z "${hf_repo}" ] && \
