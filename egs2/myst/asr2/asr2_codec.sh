...
src_token_size=
src_codebook_size=
train_speech_token_scp=
valid_speech_token_scp=
test_speech_token_scp=
train_text_scp=
valid_text_scp=
test_text_scp=
src_codebook_paths=
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    if [ "${tokenization_choice}" == "ssl" ]; then
        ...
        mkdir -p ${data_extract}/FSQ/${src_lang}/${train_set}
        mkdir -p ${data_extract}/FSQ/${src_lang}/${_dev_set}

        for dset in ${test_sets}; do
            mkdir -p ${data_extract}/FSQ/${src_lang}/${dset}
        done
        ...
        if [ ! -f "${km_dir}/"distinct_cjk_token_lists ]; then
            ...
            python3 -c "for i in range(10000): print(i, chr(int('4e00', 16) + i))" \
        ...
        # for dset in "${train_set}" ${train_sp_sets} "${_dev_set}" ${test_sets}; do
        #     cp ${data_extract}/FSQ/${src_lang}/${dset}/text \
        #         ${data_extract}/FSQ/${src_lang}/${dset}/text.${tgt_case}.${tgt_lang}
        # done

        if ${eval_valid_set}; then
            utils/copy_data_dir.sh --validate_opts --non-print ${data_audio}/org/${valid_set} \
                ${data_extract}/FSQ/${src_lang}/${valid_set}
            cp ${data_extract}/FSQ/${src_lang}/org/${valid_set}/text.${src_case}.${src_lang} \
                ${data_extract}/FSQ/${src_lang}/${valid_set}
            cp ${data_extract}/FSQ/${src_lang}/org/${valid_set}/text.${tgt_case}.${tgt_lang} \
                ${data_extract}/FSQ/${src_lang}/${valid_set}

            utils/fix_data_dir.sh --utt_extra_files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
                "${data_extract}/FSQ/${src_lang}/${dset}"
        fi

        if [ -n "${speed_perturb_factors}" ]; then
            _dirs="${data_extract}/FSQ/${src_lang}/${train_set} "
            for factor in ${speed_perturb_factors}; do
                if python3 -c "assert ${factor} != 1.0" 2>/dev/null; then
                    _dirs+="${data_extract}/FSQ/${src_lang}/${train_set}_sp${factor} "
                fi
            done
            utils/combine_data.sh \
                --extra_files "feats.scp utt2num_frames text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
                "${data_extract}/FSQ/${src_lang}/${train_set}_sp" ${_dirs}
        fi
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            for extra_file in ${utt_extra_files}; do
                ...
                for single_file in "${data_extract}/FSQ/${src_lang}/${dset}"/*; do
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    # Get the minimum number among ${nj} and the number lines of input files
    _nj=$(min "${nj}" "$(<${train_speech_token_scp} wc -l)" "$(<${valid_speech_token_scp} wc -l)")

    key_file="${train_speech_token_scp}"
    split_scps=""
    for n in $(seq "${_nj}"); do
        split_scps+=" ${_logdir}/train.${n}.scp"
    done
    ...
    key_file="${valid_speech_token_scp}"
    split_scps=""
    for n in $(seq "${_nj}"); do
    ...
    # shellcheck disable=SC2046,SC2086
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        ${python} -m espnet2.bin.mt_codec_train \
            --collect_stats true \
            --use_preprocessor true \
            --bpemodel "${tgt_bpemodel}" \
            --token_type "${tgt_token_type}" \
            --token_list "${tgt_token_list}" \
            --src_token_size "${src_token_size}"   \
            --src_codebook_size "${src_codebook_size}"  \
            --src_codebook_paths ${src_codebook_paths}    \
            --non_linguistic_symbols "${nlsyms_txt}" \
            --cleaner "${cleaner}" \
            --g2p "${g2p}" \
            --train_data_path_and_name_and_type "${train_text_scp},text,text" \
            --train_data_path_and_name_and_type "${train_speech_token_scp},src_text,text_int" \
            --valid_data_path_and_name_and_type "${valid_text_scp},text,text" \
            --valid_data_path_and_name_and_type "${valid_speech_token_scp},src_text,text_int" \
            --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    <"${asr_stats_dir}/train/src_text_shape" \
        awk -v N="${src_token_size}" '{ print $0 "," N }' \
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    if [ "${num_splits_asr}" -gt 1 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_split_dir}/speech_token_debug,src_text,text_int "
        ...
        _opts+="--train_data_path_and_name_and_type ${train_speech_token_scp},src_text,text_int "
        _opts+="--train_data_path_and_name_and_type ${train_text_scp},text,text "
...
if [ ${stage} -le 14 ] && [ ${stop_stage} -ge 14 ] && ! [[ " ${skip_stages} " =~ [[:space:]]14[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        _scp=text.${src_case}.${src_lang}
        key_file="${test_speech_token_scp}"

        # 1. Split the key file
        split_scps=""
        _nj=$(min "${inference_nj}" "$(<${key_file} wc -l)")
        asr_inference_tool="espnet2.bin.mt_codec_inference"
        ...
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            ${python} -m ${asr_inference_tool} \
                ...
                --ngpu "${_ngpu}" \
                --data_path_and_name_and_type "${test_speech_token_scp},src_text,text_int" \
