...
from espnet2.asr.discrete_asr_espnet_model_mi import ESPnetDiscreteASRModel
...
from espnet2.asr.encoder.e_branchformer_encoder_mi_film import EBranchformerEncoderMIFILM
from espnet2.asr.encoder.e_branchformer_encoder_mi_cat import EBranchformerEncoderMICAT
...
encoder_choices = ClassChoices(
    ...
    classes=dict(
        ...
        e_branchformer_mi_film=EBranchformerEncoderMIFILM,
        e_branchformer_mi_cat=EBranchformerEncoderMICAT,
    ),
...
class MTTask(AbsTask):
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        required += ["src_token_list1", "src_token_list2", "token_list"]
    ...
    def build_preprocess_fn(
        ...
        if args.use_preprocessor:
            retval = MutliTokenizerCommonPreprocessor(
                train=train,
                token_type=[args.token_type, args.src_token_type1, args.src_token_type2],
                token_list=[args.token_list, args.src_token_list1, args.src_token_list2],
                bpemodel=[args.bpemodel, args.src_bpemodel1, args.src_bpemodel2],
                non_linguistic_symbols=args.non_linguistic_symbols,
                text_cleaner=args.cleaner,
                g2p_type=args.g2p,
                text_name=["text", "src_text1", "src_text2"],
                tokenizer_encode_conf=(
                    [
                        args.tokenizer_encode_conf,
                        args.src_tokenizer_encode_conf1,
                        args.src_tokenizer_encode_conf2,
                    ]
                    if train
                    else [dict(), dict()]
                ),
    ...
    def required_data_names(
        ...
        if not inference:
            retval = ("src_text1", "src_text2", "text")
        else:
            # Recognition mode
            retval = ("src_text1","src_text2",)
        return retval
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetMTModel:
        ...
        if args.src_token_list1 is not None:
            if isinstance(args.src_token_list1, str):
                with open(args.src_token_list1, encoding="utf-8") as f:
                    src_token_list1 = [line.rstrip() for line in f]

                # Overwriting src_token_list to keep it as "portable".
                args.src_token_list1 = list(src_token_list1)
            elif isinstance(args.src_token_list1, (tuple, list)):
                src_token_list1 = list(args.src_token_list1)
            else:
                raise RuntimeError("token_list must be str or list")
            src_vocab_size = len(src_token_list1)
            logging.info(f"Source vocabulary size: {src_vocab_size }")
        else:
            src_token_list1, src_vocab_size = None, None
        ...
        model = model_class(
            ...
            src_token_list=src_token_list1,
            **args.model_conf,
            **extra_model_conf,
        )
