...
class EBranchformerEncoderMICAT(AbsEncoder):
    ...
    def __init__(
        ...
        if input_layer == "linear":
            ...
            self.embed = torch.nn.Sequential(
                torch.nn.Embedding(2*input_size, output_size, padding_idx=padding_idx),
                pos_enc_class(output_size, positional_dropout_rate, max_pos_emb_len),
            )
            ...
            if 2*input_size == output_size:
                self.embed = torch.nn.Sequential(
                    pos_enc_class(output_size, positional_dropout_rate, max_pos_emb_len)
                )
            else:
                self.embed = torch.nn.Linear(2*input_size, output_size)
        else:
    ...
    def forward(
        ...
        xs_pad1 = rnn_utils.pad_sequence(truncated_xs1, batch_first=True, padding_value=0)
        xs_pad2 = rnn_utils.pad_sequence(truncated_xs2, batch_first=True, padding_value=0)

        ilens = min_lens
        ilens2 = min_lens

        xs_pad = torch.cat((xs_pad1, xs_pad2), dim=-1)
