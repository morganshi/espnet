...
class EBranchformerEncoderMIFILM(AbsEncoder):
    ...
    def __init__(
        ...
        self.r = torch.nn.Linear(output_size, output_size)

        self.h = torch.nn.Linear(output_size, output_size)
    ...
    def forward(
        ...
        """
        batch = xs_pad.shape[0]
        min_lens = torch.min(ilens, ilens2)
        ...
        for i in range(batch):
            t = min_lens[i].item()
            truncated_xs1.append(xs_pad[i, :t, :])
            truncated_xs2.append(xs_pad2[i, :t, :])

        
        xs_pad = rnn_utils.pad_sequence(truncated_xs1, batch_first=True, padding_value=0)
        xs_pad2 = rnn_utils.pad_sequence(truncated_xs2, batch_first=True, padding_value=0)
        ...
        if (
            ...
            xs_pad2, _ = self.embed(xs_pad2, masks)

        elif self.embed is not None:
            xs_pad = self.embed(xs_pad)
            xs_pad2 = self.embed(xs_pad2)

        xs_pad2_r = self.r(xs_pad2[0])
        xs_pad2_h = self.h(xs_pad2[0])
        ...
        if len(self.interctc_layer_idx) == 0:
            if max_layer is not None and 0 <= max_layer < len(self.encoders):
                ...
                for layer_idx, encoder_layer in enumerate(self.encoders):
                    xs_pad = (xs_pad[0] + xs_pad[0] * xs_pad2_r + xs_pad2_h, xs_pad[1])
                    xs_pad, masks = encoder_layer(xs_pad, masks)
        else:
            for layer_idx, encoder_layer in enumerate(self.encoders):
                xs_pad, masks = encoder_layer(xs_pad, masks)
