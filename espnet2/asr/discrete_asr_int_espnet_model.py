...
class ESPnetDiscreteASRModel(ESPnetMTModel):
    ...
    def __init__(
        ...
        self.src_codebook_size = src_codebook_size
    ...
    def _extract_feats(
        ...
        src_text_lengths = src_text_lengths // self.src_codebook_size
        max_len = src_text_lengths.max()

        src_text_list=[]
        if self.src_codebook_size > 1:
            for i in range(batch_size):
                src_token_len = src_text_lengths[i]
                src_token = src_text[i][:self.src_codebook_size*src_token_len].reshape(self.src_codebook_size,src_token_len)
                padding_size = max_len - src_token_len
                if padding_size > 0:
                    padded_src_token = F.pad(src_token, (0, padding_size), mode='constant', value=-1)
                else:
                    padded_src_token = src_token
                src_text_list.append(padded_src_token)
            src_text = torch.cat(src_text_list, dim=0)
        ...
        #         src_token_len = src_text_lengths[i]
        #         padding_size = max_len - src_token_len
        #         src_token = src_text[i][:src_token_len]
        #         if padding_size > 0:
        #             padded_src_token = F.pad(src_token, (0, padding_size), mode='constant', value=self.src_vocab_size)
        #         else:
        #             padded_src_token = src_token
        #         src_text_list.append(padded_src_token)
        ...
        if self.frontend is not None:
            ...
            if self.src_codebook_size > 1:
                assert feats.shape[0] == feats_lengths.shape[0]*self.src_codebook_size
                feats = feats.reshape(feats_lengths.shape[0], self.src_codebook_size, max_len, -1)
                feats = feats.mean(dim=1, keepdim=False)
        else:
