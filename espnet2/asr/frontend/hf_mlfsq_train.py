...
class HfCTCMLFSQFrontend(AbsFrontend):
    ...
    def __init__(
        ...
        self.fusion_weights = torch.nn.Parameter(torch.ones(25))
    ...
    def forward(
        ...
        feats_ml = torch.stack(outputs[-1], dim=0)

        normalized_weights = torch.softmax(self.fusion_weights, dim=0)

        feats = torch.sum(
            feats_ml * normalized_weights.view(-1, 1, 1, 1), dim=0
        )
