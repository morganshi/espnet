...
class HfCTCKMFSQFrontend(AbsFrontend):
    ...
    def __init__(
        ...
        self.r = torch.nn.Linear(self.upstream.config.hidden_size, self.upstream.config.hidden_size)

        self.h = torch.nn.Linear(self.upstream.config.hidden_size, self.upstream.config.hidden_size)
    ...
    def forward(
        ...
        min_lens = torch.min(input_km_id_lengths, feats_lens)
        ...
        for i in range(batch):
            t = min_lens[i].item()
            truncated_km.append(km_feats[i, :t, :])
            truncated_fsq.append(feats[i, :t, :])

        # import ipdb;ipdb.set_trace()
        
        km_feats = rnn_utils.pad_sequence(truncated_km, batch_first=True, padding_value=0)
        feats = rnn_utils.pad_sequence(truncated_fsq, batch_first=True, padding_value=0)

        km_feats_r = self.r(km_feats)
        km_feats_h = self.h(km_feats)

        feats = feats + feats * km_feats_r + km_feats_h
