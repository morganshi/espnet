...
class HfCTCFBFSQFrontend(AbsFrontend):
    ...
    def __init__(
        ...
        self.fbank = DefaultFrontend(n_fft=1024, win_length=640, hop_length=320)

        self.r = torch.nn.Linear(80, self.upstream.config.hidden_size)

        self.h = torch.nn.Linear(80, self.upstream.config.hidden_size)
    ...
    def forward(
        ...
        fbank_feats, fbank_lens = self.fbank(input, input_lengths)

        min_lens = torch.min(fbank_lens, feats_lens)
        ...
        for i in range(batch):
            t = min_lens[i].item()
            truncated_fbank.append(fbank_feats[i, :t, :])
            truncated_wavlm.append(feats[i, :t, :])
        
        fbank_feats = rnn_utils.pad_sequence(truncated_fbank, batch_first=True, padding_value=0)
        feats = rnn_utils.pad_sequence(truncated_wavlm, batch_first=True, padding_value=0)

        fbank_r = self.r(fbank_feats)
        fbank_h = self.h(fbank_feats)

        feats = feats + feats * fbank_r + fbank_h
