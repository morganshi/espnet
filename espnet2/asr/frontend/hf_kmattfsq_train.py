...
class HfCTCKMATTFSQFrontend(AbsFrontend):
    ...
    def __init__(
        ...
        self.ffn = torch.nn.Linear(self.upstream.config.hidden_size, self.upstream.config.hidden_size)

        self.norm1 = LayerNorm(self.upstream.config.hidden_size)

        self.norm2 = LayerNorm(self.upstream.config.hidden_size)
    ...
    def forward(
        ...
        feats_norm = self.norm1(feats)

        feats_att, _ = self.cross_attn(feats_norm, km_feats, km_feats, key_padding_mask=padding_mask)

        feats = feats + feats_att

        feats_norm = self.norm2(feats)

        feats = feats + self.ffn(feats_norm)
