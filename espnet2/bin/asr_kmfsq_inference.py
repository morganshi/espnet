...
from espnet2.tasks.asr_kmfsq import ASRTask
...
class Speech2Text:
    ...
    @torch.no_grad()
    @typechecked
    def __call__(self, speech: Union[torch.Tensor, np.ndarray], km_id: Union[torch.Tensor, np.ndarray]) -> Union[
        ListOfHypothesis,
        ...
        if isinstance(km_id, np.ndarray):
            km_id = torch.tensor(km_id)
        ...
        km_id = km_id.unsqueeze(0)
        # lengths: (1,)
        km_id_lengths = km_id.new_full([1], dtype=torch.long, fill_value=km_id.size(1))

        batch = {"speech": speech, "speech_lengths": lengths, "km_id": km_id, "km_id_lengths": km_id_lengths}
