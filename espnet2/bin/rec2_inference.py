...
class Text2Speech:
    ...
    def __init__(
        ...
        # setup model
        model, train_args = GANRECTask.build_model_from_file(
        ...
        self.preprocess_fn = GANRECTask.build_preprocess_fn(train_args, False)
    ...
    def __call__(
        self,
        speech_token: Union[torch.Tensor, np.ndarray, None] = None,
        decode_conf: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, torch.Tensor]:
        """Run text-to-speech."""
        batch = dict(speech_token=speech_token)
        ...
        output_dict = self.model.tts.inference(**batch, **cfg)
...
def inference(
    ...
    loader = GANRECTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=GANRECTask.build_preprocess_fn(text2speech.train_args, False),
        collate_fn=GANRECTask.build_collate_fn(text2speech.train_args, False),
    ...
    with open(
        ...
        for idx, (keys, batch) in enumerate(loader, 1):
            ...
            speech_lengths = batch['speech_lengths'].item()
            batch = {'speech_token': batch['speech_token']}
            ...
            wav = output_dict["wav"]
            current_length = wav.size(0)
            target_length = speech_lengths
            ...
            output_dict["wav"] = wav  # 更新output_dict中的音频
            ...
            logging.info(
                f"[Shape Alignment] {key} | target: {speech_lengths} | actual: {current_length}"
            )

            shape_writer.write(
                    f"{key} " + str(target_length) + " " + str(current_length) + "\n"
                )
