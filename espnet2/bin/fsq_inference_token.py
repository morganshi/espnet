...
class Speech2Text:
    ...
    def __call__(self, speech: Union[torch.Tensor, np.ndarray]):
        ...
        speech = speech[:, : lengths.max()]

        speech_token = self.asr_model.frontend(speech, lengths, extract_token=True)
           
        results = (speech_token)
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            if enh_s2t_task or multi_asr:
                ...
                # Normal ASR
                for n, (speech_token) in zip(
                    range(1, nbest + 1), results
                ):
                    # Create a directory: outdir/{n}best_recog
                    ibest_writer = writer[f"{n}best_recog"]

                    speech_token = speech_token.squeeze().cpu().tolist()
                    # Write the result to each file
                    ibest_writer["speech_token"][key] = " ".join(map(str, speech_token))
